...
consul-template -once \
  -template "/etc/backup/rsyncd_password_file.template:/etc/backup/rsyncd_password_file"
chmod 0600 /etc/backup/rsyncd_password_file

# create target path
mkdir -p "/tmp${BACKUP_TARGET_PATH}"
touch "/tmp${BACKUP_TARGET_PATH}/.tmp"
rsync -r \
  --password-file=/etc/backup/rsyncd_password_file \
...
if [ "$BACKUP_MODE" = "sync" ]; then
  ...
  duplicity --volsize 256 --no-encryption --full-if-older-than="1M" \
    --rsync-options="--password-file=/etc/backup/rsyncd_password_file" \
    $BACKUP_SOURCE \
